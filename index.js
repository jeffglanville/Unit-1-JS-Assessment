...
function getVehiclesCostInCreditsSumTotal(character) {
  const addAll = (accumulator, {cost_in_credit}) => {
    return accumulator + cost_in_credit;
  }
  const sumAll = character.vehicles.reduce(addAll, 0);
  return sumAll;
}
...
function getStarshipPassengerAndCrewSumTotal(character) {
  const addPass = (accumulator, num) => {
    return accumulator + num.passengers;
  }
  const addCrew = (accumulator, num) => {
    return accumulator + num.crew;
  }
  const totalPass = character.starships.reduce(addPass, 0);
  const totalCrew = character.starships.reduce(addCrew, 0);
  const totalCount = totalPass + totalCrew;
  return totalCount;
}
...
function getNthFilm(character, filmNumber) {
  if(filmNumber < 4 && filNumber > 0) {
    let characterFilm = character.Films[filmNumber -1];
